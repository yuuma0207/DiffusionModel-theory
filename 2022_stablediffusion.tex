...
\end{equation}
となり、確実に関数の値が減少する。最小化したい関数に対して、簡単な形の上界を持ってきて最小化することが勾配法の仕組みである。そのため、学習率$\eta$は
\begin{equation}
  \eta=\frac{1}{L}
\end{equation}
と求まる。
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=2,samples=300]
    ...
    \begin{scope} \clip (0,0) rectangle (4.5,3);
      ...
      \draw[magenta,very thick]plot(\x,{(3*pow(2.5-0.3,4)-16*pow(2.5-0.3,3)+18*pow(2.5-0.4,2))/25+2 + (3*4*pow(2.5-0.3,3)-16*3*pow(2.5-0.3,2)+18*2*pow(2.5-0.4,1))*(\x-2.5)/25 + 2*pow(\x-2.5,2)});
    \end{scope}
    \draw(4.5,3)node[right]{$y=f(\theta)$};
  \end{tikzpicture}
  \caption{関数を上から抑えるということ}
\end{figure}
